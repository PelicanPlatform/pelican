...
func TestObjectTransferSemantics(t *testing.T) {
	...
	t.Run("G5_get_collection_recursive_flat_under_existing_dir", func(t *testing.T) {
		...
		require.NoError(t, err,
			"G5: recursive get lays entries flat under LOCAL (no basename nesting)")
		assert.Equal(t, "A", string(gotA))
		gotB, err := os.ReadFile(filepath.Join(localDir, "b.txt"))
		require.NoError(t, err)
		assert.Equal(t, "BB", string(gotB))

		// Regression guard: no `LOCAL/g5-src/` interposed.
		_, err = os.Stat(filepath.Join(localDir, "g5-src"))
	...
	t.Run("P6_put_directory_recursive_to_existing_collection_lays_flat", func(t *testing.T) {
		...
		_, err := client.DoPut(ft.Ctx, localSrc, remoteBase+"/p6-coll", true)
		require.NoError(t, err)

		got1, err := os.ReadFile(filepath.Join(remoteColl, "leaf1.txt"))
		require.NoError(t, err, "P6: recursive put lays entries flat under existing collection")
		assert.Equal(t, "leaf1", string(got1))
		got2, err := os.ReadFile(filepath.Join(remoteColl, "sub", "leaf2.txt"))
		require.NoError(t, err)
		assert.Equal(t, "leaf2", string(got2))

		// Regression guard: no `p6-coll/<basename(localSrc)>/` interposed.
		nestedBasename := filepath.Base(localSrc)
		_, err = os.Stat(filepath.Join(remoteColl, nestedBasename))
		assert.True(t, os.IsNotExist(err),
