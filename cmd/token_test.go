...
func TestTokenCreateKidFlagOverridesJWS(t *testing.T) {
	...
	// Reset every flag we touch (and the package-level scope-flag bools)
	// after the test so that leaked state doesn't affect later tests.
	t.Cleanup(func() {
		...
		readFlag, writeFlag, modifyFlag = false, false, false
	})
