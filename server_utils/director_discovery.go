...
func doDiscovery(ctx context.Context, isDirector bool) (endpoints []server_structs.DirectorAd, err error) {
	...
	if isDirector {
		...
		if err == nil && endpoint.Port() != "0" {
			...
			log.Errorln("Ignoring URL", adUrl, "specified in Director.AdvertiseUrl due to parsing error:", err)
		} else {
			log.Warningln("Ignoring URL", adUrl, "specified in Director.AdvertiseUrl as the port is set to 0")
		}
	}
