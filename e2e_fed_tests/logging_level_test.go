...
func TestCLILoggingLevelChanges(t *testing.T) {
	...
	require.NoError(t, param.Set(param.IssuerKeysDirectory, param.IssuerKeysDirectory.GetString()))
	require.NoError(t, param.Set(param.Server_ExternalWebUrl, srvURL))
	require.NoError(t, param.Set(param.Federation_DiscoveryUrl, param.Federation_DiscoveryUrl.GetString()))
