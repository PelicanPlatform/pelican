	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			if tc.shouldSetSitename {
				require.NoError(t, param.Set(param.Xrootd_Sitename, "fed-test"))
			}

			require.NoError(t, param.Set(param.Director_RegistryQueryInterval, "1s"))
			_ = fed_test_utils.NewFedTest(t, bothPubNamespaces)
...
func TestDirectorMetadataHosting(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			require.NoError(t, param.Set(param.Director_EnableFederationMetadataHosting, tt.enableHosting))
			_ = fed_test_utils.NewFedTest(t, bothPubNamespaces)
