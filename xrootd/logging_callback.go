...
func RegisterXrootdLoggingCallback(serverCtx context.Context) {
	if xrootdCallbackRegd {
		return
	}
	param.RegisterCallback("xrootd-logging", func(oldConfig, newConfig *param.Config) {
		handleXrootdLoggingChange(serverCtx, oldConfig, newConfig)
	})
	xrootdCallbackRegd = true
}

func handleXrootdLoggingChange(serverCtx context.Context, oldConfig, newConfig *param.Config) {
	...
	_, err := restartXrootdFn(ctx, serverCtx, nil)
	if err != nil {
