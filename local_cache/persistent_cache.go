...
func (pc *PersistentCache) HeadObject(objectPath, token string) (*HeadResult, error) {
	...
	if ft := pc.getFedToken(); ft != "" {
		opts = append(opts, client.WithFedToken(client.StaticTokenProvider(ft)))
	}
...
func (pc *PersistentCache) performDownload(ctx context.Context, dl *persistentDownload, token string) error {
	...
	if fedToken != "" {
		transferOpts = append(transferOpts, client.WithFedToken(client.StaticTokenProvider(fedToken)))
	}
	...
	if metadataReceived {
		...
		if metadata.ContentLength < int64(pc.storage.InlineMaxBytes()) {
