...
func setupWebUIEnv(t *testing.T) {
	...
	require.NoError(t, param.Set(param.ConfigDir, testCfgDir))
	...
	require.NoError(t, param.Set(param.Server_UIPasswordFile, tempPasswdFile.Name()))
	...
	//Override viper default for testing
	require.NoError(t, param.Set(param.IssuerKeysDirectory, filepath.Join(tempJWKDir, "issuer-keys")))
...
func TestHandleWebUIAuth(t *testing.T) {
	...
	t.Run("403-for-logged-in-non-admin-user", func(t *testing.T) {
		...
		require.NoError(t, param.Set(param.IssuerKeysDirectory, issuerDirectory))
...
func TestServerHostRestart(t *testing.T) {
	...
	require.NoError(t, param.Set(param.IssuerKey, filepath.Join(t.TempDir(), "issuer.jwk")))
