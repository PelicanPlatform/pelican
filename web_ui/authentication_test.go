	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.adminUsers != nil {
				require.NoError(t, param.Set(param.Server_UIAdminUsers, tc.adminUsers))
			}

			// Setup admin groups config
			// Only set if explicitly provided (nil means not set, empty slice means set but empty)
			if tc.adminGroups != nil {
				require.NoError(t, param.Set(param.Server_AdminGroups, tc.adminGroups))
			}
	...
	testCases := []struct {
	...
	}{
		{
			...
			setupUserFunc: func(ctx *gin.Context) {
				require.NoError(t, param.Set(param.Server_UIAdminUsers, []string{"admin1", "admin2"}))
				ctx.Set("User", "")
			},
		...
		},
		{
			...
			setupUserFunc: func(ctx *gin.Context) {
				require.NoError(t, param.Set(param.Server_UIAdminUsers, []string{}))
				ctx.Set("User", "admin")
			},
		...
		},
		{
			...
			setupUserFunc: func(ctx *gin.Context) {
				require.NoError(t, param.Set(param.Server_UIAdminUsers, []string{"admin1", "admin2", "admin3"}))
				ctx.Set("User", "admin2")
			},
			expectedCode: http.StatusOK,
		},
		{
			...
			setupUserFunc: func(ctx *gin.Context) {
				require.NoError(t, param.Set(param.Server_UIAdminUsers, []string{}))
				require.NoError(t, param.Set(param.Server_AdminGroups, []string{"pelican-admins"}))
				ctx.Set("User", "user1")
...
func TestListOIDCEnabledServersHandler(t *testing.T) {
	...
	t.Run("origin-included-if-flag-is-on", func(t *testing.T) {
		server_utils.ResetTestState()
		require.NoError(t, param.Set(param.Origin_EnableOIDC, true))
	...
	t.Run("cache-included-if-flag-is-on", func(t *testing.T) {
		server_utils.ResetTestState()
		require.NoError(t, param.Set(param.Cache_EnableOIDC, true))
	...
	t.Run("director-included-if-flag-is-on", func(t *testing.T) {
		server_utils.ResetTestState()
		require.NoError(t, param.Set(param.Director_EnableOIDC, true))
