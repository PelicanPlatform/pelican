...
func configDirectorPromScraper(ctx context.Context) (*config.ScrapeConfig, error) {
	fedInfo, err := pelican_config.GetFederation(ctx)
	if err != nil {
		return nil, errors.Wrap(err, "failed to get federation info")
	}
	discoveryUrl, err := url.Parse(fedInfo.DiscoveryEndpoint)
	if err != nil {
		return nil, errors.Wrap(err, "failed to parse the federation's resolved discovery URL")
	}
	directorUrl, err := url.Parse(fedInfo.DirectorEndpoint)
	if err != nil {
		return nil, errors.Wrap(err, "failed to parse the federation's resolved director URL")
	}

	// This prometheus token is generated by the Director on behalf of the federation.
	// It's then consumed by the Director to perform service discovery of origins/caches,
	// which are then scraped using the "scrape" token below
	...
	serverDiscoveryUrl := directorUrl
