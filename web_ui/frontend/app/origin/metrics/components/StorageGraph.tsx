...
const StorageGraph = () => {
  ...
  const { data } = useSWR<storageData>(["getStorageData", graphContext.time], () => getStorageData(graphContext.time.toSeconds()))

  const [usedBytes, freeBytes] = convertListBytes([data?.used ?? 0, data?.free ?? 0])
  ...
  const dataset = {
    ...
    datasets: [{
      data: [
        dataConvert?.used.value ?? 0,
        dataConvert?.free.value ?? 0
      ],
      backgroundColor: [
        blue[300],
        grey[50]
...
const getStorageData = async (time: number = DateTime.now().toSeconds()): Promise<{free: number, used: number}> => {
  ...
  const free = Number(result.find((r) => r.metric.type === 'free')?.value[1]) ?? 0
  const total = Number(result.find((r) => r.metric.type === 'total')?.value[1]) ?? 0
