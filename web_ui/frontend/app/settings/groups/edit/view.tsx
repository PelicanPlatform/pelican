  ...
  return (
    <>
      ...
      {isLoading || !group ? (
        ...
        <GroupForm
          group={group}
          onSubmit={async (patchedGroup: GroupPatch) => {
            setIsSubmitting(true);
            try {
              await alertOnError(
                async () => GroupService.patch(group.id, patchedGroup),
                'Error Updating Group',
                dispatch,
                true
              );
              router.push('../view/?id=' + group.id);
            } finally {
