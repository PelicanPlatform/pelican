...
import {
  ...
  LineElement,
  PointElement,
  TimeScale, Title, Tooltip, LogarithmicScale, ChartOptions, TooltipLabelStyle, TooltipItem,
} from 'chart.js';
import { useContext, useEffect, useMemo, useState } from 'react';
import { BoxAndWiskers, BoxPlotController } from '@sgratzl/chartjs-chart-boxplot';
import { MatrixResponseData, query_raw, TimeDuration, VectorResponseData } from '@/components';
import { DateTime } from 'luxon';
import { GraphContext, GraphDispatchContext } from '@/app/origin/metrics/components/GraphContext';
import useSWR from 'swr';
import { getSmallestByteCategory, toBytes, toBytesString } from '@/helpers/bytes';

ChartJS.register(BoxPlotController, BoxAndWiskers, TimeScale, CategoryScale, LinearScale, LogarithmicScale, PointElement, LineElement, Title, Tooltip, Legend, Colors, Filler);

export const BytesMetricBoxPlot = ({metric, title, options}: {metric: string, title: string, options?: ChartOptions}) => {
...
export const MetricBoxPlot = ({metric, title, options}: {metric: string, title: string, options?: ChartOptions}) => {
...
export const getMetricData = async (
  ...
  const query = `sum by (server_name) (sum_over_time(${metric}[${range}])) / sum by (server_name) (count_over_time(${metric}[${range}]))
`
