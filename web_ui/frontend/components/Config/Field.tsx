...
const Field = ({ onChange, ...props}: ParameterInputProps) => {
    ...
    switch (props.type) {
        case "bool":
            return <BooleanField onChange={handleChange<boolean>} name={props.name} value={props.Value as boolean}/>
        case "duration":
            return <DurationField onChange={handleChange<Duration>} name={props.name} value={props.Value as number}/>
        case "stringSlice":
            return <StringSliceField onChange={handleChange<string[]>} name={props.name} value={props.Value as string[]}/>
        case "string":
            return <StringField onChange={handleChange<string>} name={props.name} value={props.Value as string}/>
        case "filename":
            return <StringField onChange={handleChange<string>} name={props.name} value={props.Value as string}/>
        case "url":
            return <StringField onChange={handleChange<string>} name={props.name} value={props.Value as string}/>
        case "int":
            return <IntegerField onChange={handleChange<number>} name={props.name} value={props.Value as number}/>
        case "object":

            switch (props.name.split(".").slice(-1)[0]){
                case "Institutions":
                    return <ObjectField<Institution> onChange={handleChange<Institution[]>}  name={props.name} value={props.Value as Institution[]} Form={InstitutionForm} keyGetter={v => v.name}/>
                case "GeoIPOverrides":
                    return <ObjectField onChange={handleChange<GeoIPOverride[]>}  name={props.name} value={props.Value as GeoIPOverride[]} Form={GeoIPOverrideForm} keyGetter={v => v.ip}/>
                case "OIDCAuthenticationRequirements":
                    return <ObjectField onChange={handleChange<OIDCAuthenticationRequirement[]>}  name={props.name} value={props.Value as OIDCAuthenticationRequirement[]} Form={OIDCAuthenticationRequirementForm} keyGetter={v => v.value + v.claim}/>
                case "AuthorizationTemplates":
                    return <ObjectField onChange={handleChange<AuthorizationTemplate[]>}  name={props.name} value={props.Value as AuthorizationTemplate[]} Form={AuthorizationTemplateForm} keyGetter={v => v.prefix}/>
                case "IPMapping":
