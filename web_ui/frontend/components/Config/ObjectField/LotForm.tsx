import { Action, Lot, Path } from '@/components/Config/index';
...
const LotForm = ({ onSubmit, value }: FormProps<Lot>) => {
  ...
  const [dedicatedGB, setDedicatedGB] = React.useState<number>(
    value?.managementpolicyattrs?.dedicatedgb || 0
  );
  const [opportunisticGB, setOpportunisticGB] = React.useState<number>(
    value?.managementpolicyattrs?.opportunisticgb || 0
  );
  const [maxNumberObjects, setMaxNumberObjects] = React.useState<number>(
    value?.managementpolicyattrs?.maxnumberobjects.value || 0
  );
  const [creationTime, setCreationTime] = React.useState<number>(
    value?.managementpolicyattrs?.creationtime?.value || 0
  );
  const [expirationTime, setExpirationTime] = React.useState<number>(
    value?.managementpolicyattrs?.expirationtime?.value || 0
  );
  const [deletionTime, setDeletionTime] = React.useState<number>(
    value?.managementpolicyattrs?.deletiontime?.value || 0
  );
  ...
  return (
    <form onSubmit={submitHandler}>
      ...
      <Box mb={2}>
        <IntegerField
          onChange={setOpportunisticGB}
          name={'OpportunisticGB'}
          value={opportunisticGB}
        />
      </Box>
      <Box mb={2}>
        <IntegerField
          onChange={setMaxNumberObjects}
          name={'MaxNumObjects'}
          value={maxNumberObjects}
        />
      </Box>
