...
const ClientTable = <R extends TableData>({
  ...
  const [sort, setSort] = useState<SortState<keyof R & string> | undefined>(
  ...
  const sortedData =
    sortHandler && sort
      ? [...data].sort((a, b) => {
          const aGetter = columns[sort.columnId]?.value;
          const bGetter = columns[sort.columnId]?.value;

          const av = aGetter !== undefined ? aGetter(a) : a[sort.columnId];
          const bv = bGetter !== undefined ? bGetter(b) : b[sort.columnId];

          return sortHandler(av, bv) * (sort.direction === 'asc' ? 1 : -1);
        })
      : data;
