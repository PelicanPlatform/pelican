      ...
      components={{
        ...
        code: ({ className, children, ...props }) => {
          ...
          const inlineStyle: React.CSSProperties = {
            ...
            fontSize: '.9em',
            backgroundColor:
              theme.palette.mode == 'dark' ? '#19222d' : 'rgba(0,0,0,0.03)',
            borderWidth: '1px',
            borderColor: 'rgba(0,0,0.0.04)',
            borderRadius: '0.375rem',
            padding: '.125rem .25em .125rem .25em',
            color:
              theme.palette.mode == 'dark' ? '#e5f0fe' : 'rgba(51,65,85,1)',
          };

          // If it has no language tag, it's inline code
          if (!match) {
            return (
              <code style={inlineStyle} {...props}>
                {children}
              </code>
            );
          }
        ...
        ul: ({ children }) => (
          <List sx={{ listStyleType: 'disc' }} style={{ paddingLeft: '1rem' }}>
            {children}
          </List>
        ),
        a: ({ children, href }) => {
