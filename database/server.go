...
func serverTypeFromFlags(isOrigin, isCache bool) string {
	if isOrigin && isCache {
		return "origin_cache"
	}
	if isOrigin {
		return strings.ToLower(server_structs.OriginType.String())
	}
	if isCache {
		return strings.ToLower(server_structs.CacheType.String())
	}
	return "unknown"
}

// Create or update a record to sync local server metadata with the Registry
...
// 2) If a row with that server ID exists, it updates the existing entry.
