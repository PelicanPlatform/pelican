...
// A helper function to convert a server type into isOrigin and isCache
func typeConvertToBool(typ server_structs.ServerType) (isOrigin bool, isCache bool) {
	if typ.IsEnabled(server_structs.OriginType) {
		isOrigin = true
	}
	if typ.IsEnabled(server_structs.CacheType) {
		isCache = true
	}
	return isOrigin, isCache
}

// Create or update a record for the given server id
// Server id is an unique 7 characters string randomly generated by the server itself
// during initial registration, consisting of [0-9a-z], e.g. 18f1jk5
// Server name is a human-friendly name set by the admin via SiteName field in webUI
// or Xrootd.Sitename in local config during initial registration, e.g. "UW_OSDF_CACHE"
// 1) If no such row exists, it inserts a new one.
// 2) If a row with that name exists, it updates updated_at (and type).
func UpsertServerLocalMetadata(serverName string, id string, typ server_structs.ServerType) error {
	now := time.Now()
	isOrigin, isCache := typeConvertToBool(typ)
	...
	entry.UpdatedAt = now
	entry.IsOrigin = isOrigin
	entry.IsCache = isCache
...
func GetServerLocalMetadata() (server_structs.ServerLocalMetadata, error) {
	...
	if err != nil {
		...
		return server_structs.ServerLocalMetadata{}, err
	}
	return entry, nil
}
...
func GetServerLocalMetadataHistory() ([]server_structs.ServerLocalMetadata, error) {
	var entries []server_structs.ServerLocalMetadata
...
func SoftDeleteServerLocalMetadata(id string) error {
	...
	result := ServerDatabase.Model(&server_structs.ServerLocalMetadata{}).
