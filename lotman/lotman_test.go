...
func setupLotmanFromConf(t *testing.T, readConfig bool, name string, discUrl string, nsAds []server_structs.NamespaceAdV2) (bool, func()) {
	...
	// Load in our config
	param.Cache_HighWaterMark.Set("100g")
	param.Cache_LowWatermark.Set("50g")
	param.Logging_Level.Set("debug")
	if readConfig {
	...
	param.Lotman_LotHome.Set(tmpPath)
...
func TestLotmanInit(t *testing.T) {
	...
	t.Run("TestGoodInit", func(t *testing.T) {
		...
		param.Cache_DataLocations.Set([]string{})
		...
		param.Federation_DiscoveryUrl.Set(server.URL)
...
func TestGetLotmanLib(t *testing.T) {
	...
	param.Lotman_LibLocation.Set("/not/a/pathlibLotMan.so")
	libLoc = getLotmanLib()
	require.Equal(t, "/usr/lib64/libLotMan.so", libLoc)
...
func TestLotValidation(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			server_utils.ResetTestState()
			defer server_utils.ResetTestState()
			param.Cache_HighWaterMark.Set(tc.hwm)
...
func TestDivideRemainingSpace(t *testing.T) {
	...
	param.Cache_HighWaterMark.Set("25g")
	...
	param.Cache_HighWaterMark.Set("1g")
	lotMap = createLotMap()
	err = divideRemainingSpace(&lotMap, totalDiskSpaceB)
	require.Error(t, err)
}
...
func TestConfigLotTimestamps(t *testing.T) {
	...
	param.Lotman_DefaultLotExpirationLifetime.SetString("24h")
	param.Lotman_DefaultLotDeletionLifetime.SetString("48h")
