...
\<ImageRow alt={"Pelican and OSDF"} src={"/pelican/arch-repo-and-consumer.png"} imageContainerProps={{md: 12}} textContainerProps={{md: 12}}> The federation's core goal is connecting data owners and data consumers. </ImageRow>
...
\<ImageRow alt={"Pelican and OSDF"} src={"/pelican/arch-origin-registration.png"} imageContainerProps={{md: 12}} textContainerProps={{md: 12}}> The Origin's owner configures a federation root before starting the service. After startup, the Origin then discovers the hostnames for its Registry and Directory by using the federation root to construct the URL "https://osg-htc.org/.well-known/pelican-configuration", the federation's *discovery endpoint* containing a JSON that details the federation's central services.
