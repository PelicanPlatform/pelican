...
or OSDF). This is because the underlying cache mechanism, powered by [XRootD](https://xrootd.org/), will deliver whatever object it already has access to; if
...
### Collections
A *collection* is a grouping of objects within a namespace, analogous to a directory on a filesystem.
Collections may contain objects as well as other collections (called *subcollections*).
Unlike directories, collections do not carry filesystem metadata such as owners or permissions.
They exist purely as organizational structure within the federation's namespace.
For example, in the object path `/pelicanplatform/test/hello-world.txt`, the paths `/pelicanplatform` and `/pelicanplatform/test` are both collections.
...
[XRootD](https://xrootd.org). The namespace path, also called the *federation prefix*, is the path at which data from the Origin can be accessed in the
...
Origins, Caches build on top of [XRootD's "Proxy Storage Services."](https://xrootd.web.cern.ch/doc/dev6/pss_config.htm)
