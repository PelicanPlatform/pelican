...
The Pelican [*Registry*](/federation-services/about-pelican/core-concepts#registry-service) manages namespace registration and public key distribution. An Origin or Cache will automatically attempt to register their namespace when they are started. A federation user may also register a namespace without running a storage server by visiting the registry website through `https://<your-registry.com>`.
...
If your server's clock is not synchronized, tokens may be rejected and your Registry may fail to operate correctly.
...
Now you want to initialize the admin account for your registry web UI. You may refer to [Login to Admin Website](/sharing-your-data/federating-your-data#login-to-admin-website) for details.
...
When a user wants to register a namespace in the registry web UI, they must specify which institution this namespace is for. This is a list of options the Registry admin needs to provide. To do so you may either feed a list of `name` and `id` pairs of available institutions to register to [`Registry.Institutions`](/federation-services/configuration#Registry-Institutions) or, if you already have a web endpoint to serve such data, you may pass the URL to [`Registry.InstitutionsUrl`](/federation-services/configuration#Registry-InstitutionsUrl).
