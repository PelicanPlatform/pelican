...
The Pelican [*Director*](/federation-services/about-pelican/core-concepts#director-service) handles data distribution in a Pelican federation. It directs object requests from Pelican Clients to the proper object provider (which can be an Origin or a Cache). It also maintains a collection of actively running Origin/Cache servers in the federation.
...
If your server's clock is not synchronized, tokens may be rejected and your Director may fail to operate correctly.
