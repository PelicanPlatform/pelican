...
The Pelican [*Cache*](/federation-services/about-pelican/core-concepts#caches) creates a cache that connects to a Pelican data federation to allow faster data access. It stores data that is accessed via a Pelican origin until it gets evicted. The Cache has the potential to both be faster and physically closer to your location than the Pelican Origin you access the data from. This allows for faster data access when running complex workflows.
...
If you haven't installed Pelican, follow the instructions to [install pelican](/federation-services/install).

For _Linux_ users, it is recommended to install Pelican using one of the package managers (RPM, APK, Deb, etc.) so that Pelican dependencies are automatically handled. You may also run a [Pelican Docker image](/federation-services/install/docker) to serve a Pelican cache.
...
For _macOS_ and _Windows_ users who want to serve a Pelican cache, please use [Pelican Docker image](/federation-services/install/docker).
...
See [Server Prerequisites](/federation-services/install#enable-time-synchronization) for setup instructions.
...
You may change the port numbers through the [configuration file](/federation-services/configuration) with parameter [`Cache.Port`](/federation-services/configuration#Cache-Port) and [`Server.WebPort`](/federation-services/configuration#Server-WebPort) respectively.

### Find a federation to join

Before serving a cache, you need to find a Pelican federation to join in. If you are unfamiliar with the term **federation**, refer to [Core Concepts and Terminology](/federation-services/about-pelican/core-concepts#federations) before proceeding.
...
There are other configurations available to modify via the configuration file. Refer to the [Parameters](/federation-services/configuration) page for details.
...
1. Have data available and accessible via an origin. Since a cache only works with data available via an origin that’s in the same Pelican federation, the following assumes that you have a an accessible namespace within the same Pelican federation. For more information on how to set that up, refer to [Federating Your Data](/sharing-your-data/federating-your-data) page. The following also assume that the object to test is public file. To set up a public file through a Pelican origin, refer to the [Parameters](/federation-services/configuration#Origin-Exports) page.
