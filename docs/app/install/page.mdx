...
import DownloadsComponent from "/components/DownloadsComponent"; import { Callout } from 'nextra/components'
...
Pelican executables can run as a **client** or a **server**, and both are packaged in the same executable. However, if you intend to run a server, some non-RPM installations may require additional package dependencies. For this use case (hosting an Origin/Cache, starting your own federation Central Services) we recommend using a containerized version of Pelican from our [Pelican Docker images](./install/docker.mdx).
...
If you need a specific version that isn't the latest, you can select it using the chart's "Version" dropdown. Pelican follows the [semantic versioning scheme](https://semver.org/), so, for example, 7.10.5 means:
