...
The table below shows the minimum recommended volume mounts for each server type:
...
| **Origin** | Above + <br/> `-v /host/data:/data` (your data directory) |
| **Cache** | Above + <br/> `-v /host/cache/storage:/var/cache/pelican` (cache storage) |
| **Director** | Above + <br/> `-v /host/pelican/maxmind:/var/cache/pelican/maxmind` (optional, for GeoIP) |
| **Registry** | Above (no additional mounts required) |

> **Important**: The `/etc/pelican` directory contains cryptographic keys that establish your server's identity in the federation.
> Loss of these keys will cause the "namespace is registered with a different key" error for Origins.
...
For mounting the entire directory:
```bash
-v /host/pelican/issuer-keys:/etc/pelican/issuer-keys
```

For mounting individual key files (useful in Kubernetes with sealed secrets):
```bash
-v /host/pelican/issuer.jwk:/etc/pelican/issuer-keys/issuer.jwk
```
...
For Origins, this is the source of the "namespace is registered with a different key" error.
...
This cannot be fixed without help from the federation administrators.
...
For mounting individual database files (useful in Kubernetes):
```bash
...
For mounting individual password files (useful in Kubernetes):
```bash
...
For mounting the entire certificates directory:
```bash
-v /host/pelican/certificates:/etc/pelican/certificates
```

For mounting individual certificate files (useful in Kubernetes with sealed secrets):
```bash
...
**Volume Mount**: Included in the `/etc/pelican` mount above, or mount individually:
```bash
-v /host/session-secret:/etc/pelican/session-secret
```
...
**Configuration**: To enable file logging, see the [`Logging.LogLocation`](../parameters.mdx#Logging-LogLocation) parameter.
...
For mounting the entire directory (includes these files):
```bash
-v /host/pelican/config:/etc/pelican
```

For mounting individual credential files (useful in Kubernetes with sealed secrets):
```bash
...
**Volume Mount**: Included in the `/var/lib/pelican` mount above, or mount individually:
```bash
-v /host/pelican/origin.sqlite:/var/lib/pelican/origin.sqlite
```
...
Note: All Origin configuration, including exports and federation URL, should be provided via `/etc/pelican/pelican.yaml` which is mounted into the container.
...
**Volume Mount**: Included in the `/var/lib/pelican` mount above, or mount individually:
```bash
-v /host/pelican/cache.sqlite:/var/lib/pelican/cache.sqlite
```

**Configuration**: To customize the path, see the [`Cache.DbLocation`](../parameters.mdx#Cache-DbLocation) parameter.
...
**Why it must persist**: Without persistence, the Cache loses all cached objects on restart, defeating its purpose.
The Cache will need to re-download all data, increasing load on Origins and slowing down data access.
...
**Volume Mount**: Included in the `/var/lib/pelican` mount above, or mount individually:
```bash
-v /host/pelican/director.sqlite:/var/lib/pelican/director.sqlite
```

**Configuration**: To customize the path, see the [`Director.DbLocation`](../parameters.mdx#Director-DbLocation) parameter.
...
**Purpose**: Contains all namespace registrations, including associated public keys and metadata.
This is the most critical data for a Registry.

**Default Path**: `/var/lib/pelican/pelican.sqlite` (uses `Server.DbLocation`)
...
**Configuration**: Uses [`Server.DbLocation`](../parameters.mdx#Server-DbLocation) by default.

**Why it must persist**: The Registry database contains the authoritative record of all namespace registrations.
Loss of this database means loss of all registration data and will require namespace owners to re-register.
...
Note: All Registry configuration, including federation URL, should be provided via `/etc/pelican/pelican.yaml` which is mounted into the container.
