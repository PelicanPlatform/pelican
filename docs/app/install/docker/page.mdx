...
| **Origin** | Above + <br/> `-v /host/data:/data:ro` (your data directory) |
...
> **Important**: The `/etc/pelican` directory contains cryptographic keys that establish your server's identity in the federation. Loss of these keys will cause the "namespace is registered with a different key" error for origins.
