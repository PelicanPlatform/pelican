...
For the granting itself, see [Managing Access to a Collection](/sharing-your-data/collections-and-shares).
For the model behind these tasks — what a user is, what a group is, and how a scope reaches a caller — read [How Access Control Works](/sharing-your-data/access-control-model).
For the authoritative "who may do this", read the [Permissions Reference](/sharing-your-data/permissions-reference).
...
- **Command line:** a shell on the server host. The `pelican-server server user` and `pelican-server server group` commands drive the local server's admin API with a token they mint from the server's own issuer key, so they only work where that key is readable, and they read `Server.ExternalWebUrl` to find the API. Full flag lists live in the generated reference for [`server user`](/federation-services/commands-reference/pelican-server/server/user) and [`server group`](/federation-services/commands-reference/pelican-server/server/group).
- **API:** a login cookie or an API token whose creator holds that scope. The Swagger browser at [`/api-docs`](/sharing-your-data/api) carries every request and response shape; this page never repeats them.
...
*Requires `server.admin` or `server.user_admin` — see [User administration](/sharing-your-data/permissions-reference#user-administration).*
...
New accounts are granted the scopes named in [`Server.NewUserDefaultScopes`](/sharing-your-data/configuration#Server-NewUserDefaultScopes).
...
The link's lifetime comes from [`Server.GroupInviteLinkExpiration`](/sharing-your-data/configuration#Server-GroupInviteLinkExpiration) unless you override it.
...
*Requires `server.admin` or `server.user_admin`, with limits — see [User administration](/sharing-your-data/permissions-reference#user-administration).*
...
*Any signed-in user can create a group — see [Group administration](/sharing-your-data/permissions-reference#group-administration).*
...
Once the group exists, use it to grant access to a collection: see [Grant read or write access to a group](/sharing-your-data/collections-and-shares#grant-read-or-write-access-to-a-group).
...
*Requires group ownership, a group administrator delegation, or `server.admin` — see [Group administration](/sharing-your-data/permissions-reference#group-administration).*
...
Expiry defaults to [`Server.GroupInviteLinkExpiration`](/sharing-your-data/configuration#Server-GroupInviteLinkExpiration).
...
*Requires group ownership or `server.admin` — see [Group administration](/sharing-your-data/permissions-reference#group-administration).*
...
*Turning the flag on for an existing group requires `server.admin` — see [Group administration](/sharing-your-data/permissions-reference#group-administration).*

[`Issuer.AuthorizationTemplates`](/sharing-your-data/configuration#Issuer-AuthorizationTemplates) and the `Server.*AdminGroups` settings — [`Server.AdminGroups`](/sharing-your-data/configuration#Server-AdminGroups) and [`Server.UserAdminGroups`](/sharing-your-data/configuration#Server-UserAdminGroups) among them — match on a group's *name*, and they only trust a name that carries the auth-template-eligibility flag.
...
For why the flag exists, see [Where a scope can come from](/sharing-your-data/access-control-model#where-a-scope-can-come-from).
...
*Requires `server.admin` — see [Management scopes](/sharing-your-data/permissions-reference#management-scopes).*
...
See [Sources of a scope](/sharing-your-data/permissions-reference#sources-of-a-scope) for every path a grant can arrive by and how to withdraw each one.
...
To switch the requirement off entirely, set [`Server.AUPFile`](/sharing-your-data/configuration#Server-AUPFile) to `none`.

## Troubleshooting

**`403` with `requires_aup: true`.**
You have not accepted the current Acceptable Use Policy.
Open the acceptance page, read the policy, and click **I AGREE**; see [Accept the Acceptable Use Policy](#accept-the-acceptable-use-policy) and the [Acceptable Use Policy gate](/sharing-your-data/permissions-reference#acceptable-use-policy-gate).
...
`user-` is reserved.
See [Identifier rules](/sharing-your-data/permissions-reference#identifier-rules).

**An entry in an admin list grants nothing.**
[`Server.UIAdminUsers`](/sharing-your-data/configuration#Server-UIAdminUsers) and [`Server.UserAdminUsers`](/sharing-your-data/configuration#Server-UserAdminUsers) match on usernames, not on OIDC subjects; [`Server.AdminGroups`](/sharing-your-data/configuration#Server-AdminGroups) and [`Server.UserAdminGroups`](/sharing-your-data/configuration#Server-UserAdminGroups) match on group names.
