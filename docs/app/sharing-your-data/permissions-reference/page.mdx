...
Instructions for carrying out the tasks themselves live in [Managing Users and Groups](/sharing-your-data/users-and-groups) and [Managing Access to a Collection](/sharing-your-data/collections-and-shares); the reasoning behind the model lives in [How Access Control Works](/sharing-your-data/access-control-model).
...
|---|---|---|
| System administrator | Holds `server.admin`: a direct grant, membership of an auth-template-eligible group named in [`Server.AdminGroups`](/sharing-your-data/configuration#Server-AdminGroups), a username in [`Server.UIAdminUsers`](/sharing-your-data/configuration#Server-UIAdminUsers), or the built-in `admin` account. | Every management surface on this server. Implies `server.user_admin` and `server.collection_admin`. |
| User administrator | Holds `server.user_admin`: a direct grant, or a match in [`Server.UserAdminUsers`](/sharing-your-data/configuration#Server-UserAdminUsers) / [`Server.UserAdminGroups`](/sharing-your-data/configuration#Server-UserAdminGroups). | The user surface and group listing. Cannot grant scopes, cannot manage linked identities, and cannot act on a system administrator's account. |
| Collection administrator | Holds `server.collection_admin`: a direct grant, or a match in [`Server.CollectionAdminUsers`](/sharing-your-data/configuration#Server-CollectionAdminUsers) / [`Server.CollectionAdminGroups`](/sharing-your-data/configuration#Server-CollectionAdminGroups). | Creates collections, and sees and manages every collection on the server. |
...
| Ordinary user | Any account that signs in successfully. New accounts receive [`Server.NewUserDefaultScopes`](/sharing-your-data/configuration#Server-NewUserDefaultScopes), which by default is `web_ui.access`. | Their own account, groups they create or belong to, and collections and shares they are named on. |
...
Group matches against the configuration lists and against [`Issuer.AuthorizationTemplates`](/sharing-your-data/configuration#Issuer-AuthorizationTemplates) are restricted to groups flagged auth-template-eligible.
...
| Question | Answer |
|---|---|
| Who may create a share | A caller with `read` or better on the parent collection, when the parent has sharing enabled and [`Origin.Multiuser`](/sharing-your-data/configuration#Origin-Multiuser) is off. |
...
[`Server.AUPFile`](/sharing-your-data/configuration#Server-AUPFile) set to `none` removes the requirement entirely.

## Response codes

One row per meaning, not per endpoint.
Full request and response schemas are in the [API browser](/sharing-your-data/api).
...
- [How Access Control Works](/sharing-your-data/access-control-model) — the model behind these tables.
- [Managing Users and Groups](/sharing-your-data/users-and-groups) — how to carry out the user and group tasks.
- [Managing Access to a Collection](/sharing-your-data/collections-and-shares) — how to carry out the collection and share tasks.
- [API browser](/sharing-your-data/api) — request and response schemas for every endpoint named here.
- [`pelican-server server user`](/federation-services/commands-reference/pelican-server/server/user) and [`pelican-server server group`](/federation-services/commands-reference/pelican-server/server/group) — generated CLI reference.
- [`pelican-server origin collection`](/federation-services/commands-reference/pelican-server/origin/collection) — generated CLI reference for collections.
- [Parameters](/sharing-your-data/configuration) — the admin-list settings, `Server.NewUserDefaultScopes`, `Server.AUPFile`, and `Origin.Multiuser`.
