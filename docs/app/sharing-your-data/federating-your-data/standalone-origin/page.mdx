...
When [`Origin.EnableStandaloneMode`](/sharing-your-data/configuration#Origin-EnableStandaloneMode) is set to `true`, the Origin will **not** register with a Registry, will **not** advertise itself to a Director, and will not contact any other federation service.
...
This is the Origin counterpart of a [site-local Cache](/federation-services/cache/site-local-cache), but it goes further: a site-local Cache still queries a Director as a client, whereas a standalone Origin makes no federation calls whatsoever.
...
- Standalone mode cannot be combined with the Director, Registry, Cache, or Broker modules in the same process, nor with [`Origin.DisableDirectClients`](/sharing-your-data/configuration#Origin-DisableDirectClients) -- a standalone Origin has only direct clients.
- [`Federation.DiscoveryUrl`](/sharing-your-data/configuration#Federation-DiscoveryUrl) must not be set: a standalone Origin belongs to no federation and publishes its own federation metadata, so naming a remote one would leave the process holding two different answers to "what federation is this?".
...
- [`Origin.DirectorTest`](/sharing-your-data/configuration#Origin-DirectorTest) is force-disabled, with a warning in the log, since no Director exists to run the test.
...
With no federation to discover, the Origin's own [`Server.ExternalWebUrl`](/sharing-your-data/configuration#Server-ExternalWebUrl) is the federation's identity -- that is the host clients name in a `pelican://` URL.
...
With [`Origin.EnableIssuer`](/sharing-your-data/configuration#Origin-EnableIssuer) set, each non-public export gets its own OIDC issuer at `<Server.ExternalWebUrl>/api/v1.0/issuer/ns<FederationPrefix>` -- for the export above, `https://origin.example.com:8444/api/v1.0/issuer/ns/my-data`.
That issuer is what mints the tokens clients present.

<Callout type="warning">
  Set [`Origin.IssuerMode`](/sharing-your-data/configuration#Origin-IssuerMode) to `embedded` explicitly.
...
See [Generating Tokens](/sharing-your-data/federating-your-data/generating-tokens) for how to obtain one.
...
To join a federation later, set [`Federation.DiscoveryUrl`](/sharing-your-data/configuration#Federation-DiscoveryUrl), remove `Origin.EnableStandaloneMode`, and restart; the Origin will then register its namespaces and begin advertising.
Note that the namespaces you chose while standalone must be available in the federation's Registry -- see [Choosing a Namespace Prefix](/sharing-your-data/federating-your-data/choosing-namespaces).
