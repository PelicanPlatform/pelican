...
Pelican users who want to share data within a Pelican federation do so via an [***Origin***](/sharing-your-data/about-pelican/core-concepts#origins).
...
<Callout>
Are you setting up an Origin to connect to the OSDF? If so, please read the OSDF documentation for system administrators here: [osg-htc.org/docs/data/osdf/overview/](https://osg-htc.org/docs/data/osdf/overview/).
</Callout>

## Before Starting

This guide assumes you've already installed Pelican. If you haven't, see [Installing Pelican](/sharing-your-data/install) for more information.
...
Pelican servers rely on time-sensitive tokens for authorization.
If your server's clock is not synchronized, tokens may be rejected and your Origin may fail to join the federation.
Please ensure NTP is enabled on your server.
See [Server Prerequisites](/sharing-your-data/install#enable-time-synchronization) for setup instructions.
...
Pelican has chosen ports 8443 for data transfers and 8444 for the browser interface as defaults, but you may change these port numbers through your Origin's [configuration file](/sharing-your-data/configuration) with parameters [`Server.WebPort`](/sharing-your-data/configuration#Server-WebPort) and [`Origin.Port`](/sharing-your-data/configuration#Origin-Port), respectively.
...
Before serving an Origin, you need to decide which [***federation***](/sharing-your-data/about-pelican/core-concepts#federations) your data will be accessed through. For example, the Open Science Data Federation (OSDF) is Pelican's flagship federation, and if you are interested in serving an OSDF Origin, you can refer to the [OSDF website](https://osg-htc.org/services/osdf.html) for details about how to join. If you're unsure about which federation to join and aren't ready to run your own federation, this is a good place to start.
...
For more information about how to choose prefixes, see [Choosing a Namespace Prefix](/sharing-your-data/federating-your-data/choosing-namespaces)
...
Origin functionality is affected by any configuration option in the [Configurable Parameters](/sharing-your-data/configuration#configurable-parameters) that lists `*` or `origin` as a component.
These primarily consist of:
- [Logging configuration](/sharing-your-data/configuration#logging): Used to set the logging verbosity of various Pelican/XRootD processes
- [Federation configuration](/sharing-your-data/configuration#Federation): Used to point the Origin at a specific federation
- [Origin configuration](/sharing-your-data/configuration#Origin): Used to specify exports, set up Origin access to the object store, and configure access policies and throttling
- [Server configuration](/sharing-your-data/configuration#Server): Used to modify hostname/port setup, configure the server's TLS credentials, and control other aspects of the server runtime
- [Issuer configuration](/sharing-your-data/configuration#Issuer): Used for setting up a local [OA4MP token issuer](https://oa4mp.org/) on behalf of the Origin
- [XRootD configuration](/sharing-your-data/configuration#Xrootd): Used to control the Origin's underlying XRootD instance
- [Monitoring configuration](/sharing-your-data/configuration#Monitoring): Used to tune the Origin's embedded Prometheus instance and the way it records Origin metrics
...
See [Example Configurations](/sharing-your-data/federating-your-data#example-origin-configurations) for additional examples covering how to combine these into a coherent Origin configuration.
...
These capabilities are most often configured in the [`Exports`](/sharing-your-data/configuration#Origin-Exports) section of the `Origin` configuration block, e.g.:
```yaml
Origin:
  Exports:
...
4. **Grant organization access**: If you want to restrict access based on GitHub organization membership, you must authorize the OAuth App to access those organizations.
...
Origin:
  EnableIssuer: true
  EnableOIDC: true
  # Note: Pelican's built-in OA4MP token issuer currently supports only one
  # protected (non-public) export per Origin. This limitation applies to all
  # authentication providers, including GitHub and CILogon. However, the Origin
  # can still export any number of fully-public namespaces.
  Exports:
...
However, Pelican aims to support a variety of backends and currently supports serving objects from S3 and Globus in addition to POSIX.
Configuration for these backends is mostly similar to configuration for POSIX file systems, but with a few important differences.
For details, refer to the [S3 Backend](/sharing-your-data/federating-your-data/s3-backend) and [Globus Backend](/sharing-your-data/federating-your-data/globus-backend) pages.
...
This example is more complex than the previous examples because it configures the Origin's ["Multiuser"](/sharing-your-data/configuration#Origin-Multiuser) feature and makes use of a non-local token issuer.
...
Registering a federation namespace prefix is the process of claiming the prefix with the federation's [***Registry***](/sharing-your-data/about-pelican/core-concepts#registry).
...
For more information about how to choose these prefixes, see [Choosing a Namespace Prefix](/sharing-your-data/federating-your-data/choosing-namespaces)
...
For more information about key generation, see [Advanced Usage/Server](/sharing-your-data/admin-credentials#pre-generate-credentials).
</Callout>
...
In the meantime, store your private key someplace safe -- once you're ready to start your Origin, you'll configure it to use the private key using the [`IssuerKeysDirectory`](/sharing-your-data/configuration#IssuerKeysDirectory) configuration option.
...
For more information about available yaml configuration options, refer to the [Parameters page](/sharing-your-data/configuration).
