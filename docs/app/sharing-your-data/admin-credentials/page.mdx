...
```bash copy
pelican-server key create --private-key /tmp/foo --public-key /tmp/bar
```

You can also use this command to derive the public key of an existing private key:

```bash copy
pelican-server key create --private-key /path/to/existing/private/key
```
...
The `pelican-server generate` command generates the [htpasswd](https://httpd.apache.org/docs/2.4/programs/htpasswd.html)-formatted password file for storing the web interface administrator login password. To generate a password file, run:
...
```bash copy
pelican-server generate password -o /path/to/generated-htpasswd-file -p /path/to/password
```
...
Pelican scans the above directory every 60 seconds to detect newly added or removed private keys. When [retrieving public keys](/sharing-your-data/federating-your-data/origin#obtain-the-public-key-of-the-origin), Pelican dynamically generate them from the current set of private keys on-the-fly. If `Server.IssuerJwks` is specified, Pelican also includes public keys from that `.jwks` file.
...
1. Generate a new private/public keypair by using the `pelican-server key create` command, see details [here](#namespace-private-key).
...
```bash copy
P11_KIT_DEBUG=rpc pelican-server origin serve
```
