...
1. If you haven't installed Pelican, follow the instructions to [install Pelican](../install).

2. Run your Pelican Origin server. You may run other Pelican servers, but in this documentation we will use the Origin server for demonstration. For details about serving a Pelican Origin, refer to [Federating Your Data](/sharing-your-data/federating-your-data) page.

    ```bash copy
    pelican-server origin serve -v /tmp/stash:/test-namespace -f "osg-htc.org"
    ```
...
1. Run the following command
    ...
    * Replace `--lifetime 604800` with your desired lifetime for the token. The value is in seconds. The example below generates a token with a lifetime of 7 days.
    ...
    ```bash
    $ pelican-server origin token create --scope monitoring.query --claim wlcg.ver=1.0 --issuer <server-web-url> --subject grafana --audience <server-web-url> --lifetime 604800
...
```bash copy
PELICAN_MONITORING_PROMQLAUTHORIZATION=false pelican-server origin serve
```
...
That's it! Feel free to modify the dashboard to tailor to your needs. For a list of available Prometheus metrics to query, please refer to the [previous section](./prometheus).
