...
If the remote destination names an existing collection (or you pass multiple sources), `pelican object put` infers the object name from the source for **non-recursive** uploads, matching `cp` / `scp` behaviour:
