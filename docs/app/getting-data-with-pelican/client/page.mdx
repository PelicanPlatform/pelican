...
<Callout type="warning">
When uploading, if the source is a single file and the destination path ends with `/`, Pelican interprets the destination as a collection.
If that collection does not already exist at the remote, the destination will be treated as an object name instead.
To avoid confusion, omit the trailing `/` when the destination is intended to be an object, or ensure the collection already exists.
</Callout>
