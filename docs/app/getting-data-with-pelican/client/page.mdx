...
If the remote destination names an existing collection (or you pass multiple sources), `pelican object put` infers the object name from the source, matching `cp` / `scp` behaviour:
...
Recursive uploads (`-r`) nest each source under `basename(source)` inside the destination collection:

```bash
# Contents of ./mydir land at pelican://.../<namespace>/mydir/...
pelican object put -r ./mydir pelican://<federation-url>/<namespace>
```

The same layout rule applies symmetrically to `pelican object get`: downloading into an existing local directory places the object at `<local-dir>/<basename(source)>`, and a recursive get nests under `basename(source)`.

> **Note (library callers):** the `cp`-style container-target semantics described here are applied by the CLI. Programs that call `client.DoGet` / `client.DoPut` directly (for example `pelican object sync`, or an embedding via the client library) continue to see the flat layout: a recursive `DoGet` into an existing local directory drops entries directly under it. Callers that want the CLI's nesting should compose `basename(source)` into the destination path themselves.
