...
1. Ensure that the Pelican server is running. Copy the new private key file to [IssuerKeysDirectory](#multiple-keys).
1. Visit your federation's Registry in your browser (you can find it in your Pelican server's web interface) and download the latest registered public key set for your server under "Origins" section. Ensure the new public key is included. If it isn't, wait for 60 seconds and retry.
1. Remove the old private key from [IssuerKeysDirectory](#multiple-keys)
1. Wait for 60s, then go back to the Registry, download the latest registered public key set again. Verify the old public key has been removed.
