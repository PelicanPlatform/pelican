...
You can also use this command to derive the public of an existing private key.
...
1. Generate a new pair of private and public key by the `pelican key create` command.
2. Make sure your Pelican server is running. Copy the new private key file to [IssuerKeysDirectory](#multiple-keys).
...
> If you are using HTCondor Credmon, the waiting time will be 15m OR generated token lifetime configured in the credmon.
