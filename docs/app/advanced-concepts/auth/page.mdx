...
XRootD's authorization library uses basic string matching to determine access privileges, so `/path-privileged` will match the access policy of `/path` if `/path` is checked first.
</Callout>
