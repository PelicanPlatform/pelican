...
If you operate a separate Prometheus instance and want it to scrape metrics from one or more Pelican services, you can point that Prometheus at the Pelican `/metrics` endpoint.
Because `/metrics` requires authentication by default, you must issue a Pelican API key for the scraping job and configure Prometheus to send it as a bearer credential.

The built-in Prometheus instance (controlled by `Monitoring.EnablePrometheus`, on by default) is sufficient when you only need to inspect a single server: it keeps metrics for the period set by `Monitoring.DataRetention` (15 days by default) and needs no extra infrastructure.
Use an external Prometheus when you want to collect metrics from several Pelican servers in one place, retain them for longer, or feed a shared Grafana or alerting stack.
The two are not mutually exclusive: `/metrics` is served regardless of `Monitoring.EnablePrometheus`, so you can keep the built-in instance and scrape the same server externally.

### Generate an API key

Use `pelican apikey generate` on the Pelican server you want to scrape. The command reads the server's configuration to sign its request with the server's issuer key, so if the server's configuration file is not at the default location, pass the same `--config <path>` the server was started with.
The key must include the `monitoring.scrape` scope, which grants access to the `/metrics` endpoint. (The separate `monitoring.query` scope is only needed when issuing PromQL queries against `/api/v1.0/prometheus`; it does not grant access to `/metrics`.)
...
* `--server`: must match the Pelican server's `Server.ExternalWebUrl`. When that value is not set explicitly, it falls back to `https://<Server.Hostname>[:<WebPort>]`, with the default `:443` port stripped. If the server is reachable by multiple hostnames, use the one the server is configured with. For example, if `aaa.com` is a CNAME pointing at `bbb.com` and the server's configuration sets `Server.Hostname: bbb.com` with `Server.ExternalWebUrl` unset, use `--server https://bbb.com`; if the configuration instead sets `Server.ExternalWebUrl: https://aaa.com`, use `--server https://aaa.com`. A mismatch will be rejected.
...
> **Note**: For a test deployment that uses a self-signed certificate, you can turn off server certificate verification for this job by adding the following under the job entry. Do not use this in production.
>
> ```yaml
>     tls_config:
...
Start Prometheus with the configuration file above (or reload a running instance so it picks up the new job):

```bash copy
prometheus --config.file=/etc/prometheus/prometheus.yml
```

Once Prometheus is running, the new target should appear as `UP` on the Prometheus `/targets` page (`http://localhost:9090/targets` by default).
If the target instead reports a `403`, double-check that the API key was issued with the `monitoring.scrape` scope and that the `--server` value used when generating the key matched the Pelican server's `Server.ExternalWebUrl`.
...
The previously deprecated metrics below have been removed. Use the replacement listed for each.

XRootD metrics:

- `xrootd_monitoring_packets_received`: Removed; use `xrootd_monitoring_packets_received_total`.
- `xrootd_transfer_readv_segments_count` and `xrootd_transfer_readv_segments_toal`: Removed; use `xrootd_transfer_readv_segments_total`.
- `xrootd_transfer_operations_count`: Removed; use `xrootd_transfer_operations_total`.
- `xrootd_server_connection_count`: Removed; use `xrootd_server_connections_total`.
- `xrootd_server_bytes`: Removed; use `xrootd_server_bytes_total`.
- `xrootd_server_io_wait_time`: Removed; use `xrootd_server_io_wait_seconds_total`.

Director metrics:

- `pelican_director_client_version_total`: Removed; use `pelican_director_client_requests_total`.
- `pelican_director_redirections_total`: Removed; use `pelican_director_redirects_total`.
- `pelican_director_geoip_errors` and `pelican_director_geoip_errors_total`: Removed; these were split into `pelican_director_maxmind_server_errors_total` and `pelican_director_maxmind_client_errors_total`.
