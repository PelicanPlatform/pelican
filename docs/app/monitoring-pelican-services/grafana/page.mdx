...
1. If successful, you will see a prompt to change the password.
...
1. Run your Pelican Origin server. You may run other Pelican servers, but in this documentation we will use the Origin server for demonstration. For details about serving a Pelican Origin, refer to [Federating Your Data](../federating-your-data.mdx) page.

   ```bash copy
   pelican origin serve -v /tmp/stash:/test-namespace -f "osg-htc.org"
   ```

1. Go to the URL for Web UI and confirm it's working. The default URL is `https://localhost:8444`. You should be directed to the Pelican login page.
...
1. Copy the generated token. Note that your token will be different from the example above.

1. In the Grafana data source configuration page, go to the **Authentication** section, expand the **HTTP Headers** subsection, and click **Add header**

1. Fill in `Authorization` for the **Header** field and `Bearer <token>` for the **Value** field with your generated token. Note that there is a whitespace between `Bearer` and your token.
...
1. Download the template dashboard file from [here](https://github.com/PelicanPlatform/pelican/blob/main/docs/public/origin-dashboard-template.json)
...
- Replace `<YOUR-DATA-SOURCE-UID>` to the id of your Prometheus data source. You can find the ID on the data source configuration page.
  ...
  1. Copy the ID from your browser's URL box, which should have a value similar to
  ...
  1. Batch replace `<YOUR-DATA-SOURCE-UID>` in the template dashboard file with the copied value
...
1. Click **Dashboards** in the left-side menu. If there is no menu on the left-side, click the icon of stacked bars to toggle the menu, which is located on the top-left side of the page next to **Home**

1. Click the **New** button on the top-right side. Select the **Import** option in the dropdown menu.

1. Drag and drop the template JSON file from your computer to the upload section of the page. It should look like the screenshot below.
