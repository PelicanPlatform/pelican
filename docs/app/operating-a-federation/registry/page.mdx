...
By default, a new origin can successfully *register* against the registry, but the origin is still in unapproved state. The director will not be able to direct object requests to the unapproved origin until it is approved manually by the admin. When this configuration parameter is set to `false`, any origin will be able to join without admin approval.

A registry admin may edit, review, approve, or deny a new origin registration in the registry Web UI.

### `Registry.RequireCacheApproval`

Similarly, a new cache can successfully *register* against the registry but still remains in unapproved state. The director will not redirect object requests to the unapproved cache until it is approved manually by the admin. Set this parameter to `false` if you want any cache to join without admin approval.
