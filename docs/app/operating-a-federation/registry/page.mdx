...
By default, a registry runs on port `8444`. You may change the port number by passing `-p <port>` when serving the registry or by setting `Server.WebPort` in the configuration file.
