...
The Director runs these three steps in order and stops at the first failure, so the DELETE only runs after a successful GET.
A cycle that fails after the upload therefore leaves that one test file behind on the Origin's disk.
