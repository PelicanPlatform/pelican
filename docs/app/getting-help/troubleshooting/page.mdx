...
If you are able to connect to the Director website, there should be a section called `Namespaces` where you can search for the namespaces registered with the Federation.
</Callout>

If you have used the exact same command in the past, successfully, then it is likely that the Origin service is offline or otherwise unable to connect to the Director.
...
For the purposes of caching, Pelican assumes that **the content of an object does not change** once it has been fetched.
...
If you enter the password incorrectly for your local configuration file, you should see a message suggesting that you use the command `pelican credentials reset-local` to remove your configuration file.
This is generally safe to do, unless you are actively transferring data in another process.

You can reset your local configuration at anytime by running

```
pelican credentials reset-local
```

If that doesn't work, you can also manually reset your local configuration as follows.
...
Access Points (APs) with HTCondor installed should also have the `pelican` command installed.
