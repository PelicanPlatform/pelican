...
It is the same cast as the worked example in [Managing Data Access](/managing-data-access#example), followed task by task.
...
Over the sections below, the lab's collection gets created, handed to the people who run it, and opened up to students — and one student passes part of that access on to classmates outside the lab.
...
You need an exported namespace.
A collection is access control *inside* an exported namespace, not a way to publish new storage, so export the storage first — see [Serving an Origin](/federating-your-data/origin) — then create a collection at that prefix or below it.
...
> Brian does not know the groups that will want to access collections, he will let the Reyes lab manage that after he passes it on.
...
> Christina sets up the lab's two working groups: `reyes_researchers`, for the people who collect and process data, and `reyes_students`, for the people who only analyze it.
> Danny goes in the first, Andrew in the second.
> Then Christina grants `reyes_researchers` `write` on the collection and `reyes_students` `read` — Andrew can read the processed deployment files and cannot overwrite them.
...
> Andrew wants to use part of the lab's data for a class project, alongside two classmates who have no connection to the lab.
> They need to read one folder, and Andrew is the one who should be answerable for it — not Dr. Reyes.
> Andrew cannot create a collection for them: that needs `server.collection_admin`.
> So Christina copies the example data the class needs into `/riptide-university/reyes-lab/andrews-project` and switches sharing on, which lets the collection's own readers lend out their own corners of it, on their own authority, with nobody gaining a management scope.
...
<Callout type="info">
  **A share binds data to its owner — check that is what you want.**
  Objects reached through a share are served as its *owner*, and the share is re-intersected with that owner's access to the parent on every issuance, so it shrinks and disappears as they do.
  That is the right shape when one person is lending out their own corner of a collection and should stay answerable for it.

  The deciding question is not whether the recipients are outsiders — a collection can grant a group of outsiders perfectly well.
  It is whose data this is, how many people need it, and how long it has to last.
  When the answers are the group's, several, and indefinitely, that is a collection with a group grant, and it is worth the request to whoever holds `server.collection_admin` — see [Create a collection](#create-a-collection) and [Grant read or write access to a group](#grant-read-or-write-access-to-a-group).
  A share pressed into that job makes one member a single point of failure and files every collaborator's reads under that person's name.
</Callout>
...
*Requires `read` or `write` on the parent collection, with sharing enabled on it — see [Share administration](/managing-data-access/permissions-reference#share-administration).*

> Andrew creates a share named `Class project`, scoped to the folder Christina prepared at `/riptide-university/reyes-lab/andrews-project`.
> Andrew owns it and manages it, and the classmates see that one folder and nothing else in the lab's archive.
> It also expires on its own: when Andrew's `reyes_students` grant comes off, the share stops issuing usable tokens, and nobody has to remember it existed.
...
> Andrew grants the classmates `read`, through a group made for the project or through each classmate's personal `user-` group.
> Andrew holds only `read` on the parent through `reyes_students`, so `read` is the most this share can ever hand out: setting the role to `Write` would still issue read-only tokens, because the intersection clamps it.
...
> Three things end in May, and each one calls for a different lever.
> One of Andrew's classmates drops the class — Andrew removes that person from the share's group and touches nothing else.
> The lab stops taking undergraduates, so Christina revokes the `reyes_students` read grant on the collection entirely.
> And that grant is the whole basis of Andrew's class-project share, so the share empties on its next issuance — a consequence of the second lever, with nobody editing the share at all.
...
> Dr. Reyes decides to retire a leave the lab to Christina. She must transfer the collection to Christina to enable Christina to manage her own admin users.
