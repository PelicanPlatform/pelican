...
Over the sections below, the lab's collection gets created, handed to the people who run it, and opened up to students — and one student passes part of their own access on to a group of interns.
...
| Wes Danforth | Graduate student. Processes each new deployment; supervises a group of interns. | `reyes-grads` — `write` |
...
| Sophia Adeyemi | Summer intern, working on an experimental data transformation. | `reyes-interns` — `write`, through Wes's share |
...
3. Under **Collection**, fill in:
   ...
   > Dana names it `Reyes Lab`, picks the `/riptide-university` namespace from the dropdown and types `reyes-lab` beside it, describes it as "Nearshore current data, Reyes Lab", and leaves it `Private`.
...
> They need to read the processed deployment files, and they must not be able to overwrite them.
> Dr. Raman puts both of them in `reyes-undergrads` and grants that group `read` to the collection.
...
6. Repeat for each further group you want to grant.
   ...
   > Dr. Raman adds `reyes-undergrads` with **Role** `Read`.
...
> She stays the owner — the collection is the lab's, but she uses membership to the admin group `reyes-postdocs` to enable others to manage for her.
...
> Dana created the collection before Dr. Reyes had ever signed in to the origin, which made Dana the initial owner.
...
> Wes Danforth has taken on a group of summer interns to try an experimental data transformation.
> They need somewhere to write their output, and Wes is the one who should be answerable for it — not Dr. Reyes, and certainly not Research Computing.
> Wes cannot create a collection for them: that needs `server.collection_admin`.
> So Dr. Reyes switches sharing on, which lets the collection's own readers lend out their own corners of it, on their own authority, with nobody gaining a management scope.
...
> Wes creates a share named `Intern transformations`, scoped to the folder made for them at `/riptide-university/reyes-lab/interns`.
> Wes owns it and manages it, and the interns see that one folder and nothing else in the lab's archive.
> It also expires on its own: when Wes's `reyes-grads` grant comes off, the share stops issuing usable tokens, and nobody has to remember it existed.
...
> Wes grants `reyes-interns` the `write` role, so Sophia and the other interns can write their output into the folder.
> Wes holds `write` on the parent through `reyes-grads`, so the intersection leaves that `write` intact; had Wes only held `read`, the interns would get `read` no matter what the share said.
...
> And Wes finishes and leaves, so the `reyes-grads` grant goes — which empties the intern share on its next issuance, without anyone editing the share.
