...
* Delegating group management: Controlling who has permission to make membership changes
...
Most of these tasks can also be driven from the **command line**, with the `pelican-server server user` and `pelican-server server group` commands.
Those assume you are logged into the origin's host or container and have read access to the origin's keys (typically in `/etc/pelican/issuer-keys/`) to authorize the action.
Flags and syntax live in the generated reference for [`server user`](/commands-reference/pelican-server/server/user) and [`server group`](/commands-reference/pelican-server/server/group); this page repeats them only where the web interface cannot do the job at all.

Additionally, one can use direct **API** access to invoke the same operations over the REST interface; the documentation at [`/api-docs`](/api-docs) carries every request and response shape but
