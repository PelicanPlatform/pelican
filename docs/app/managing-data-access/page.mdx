...
Everything after that — including deleting the collection — is done by the collection's owner and the people they delegate to, with no administrator privilege at all.
...
To give one person access, put them in a group — or use the implicit personal group every user has, named `user-` followed by their username, which can be granted to directly.
...
1. Dr. Reyes generates an invite link for her lab manager Christina, who redeems it and creates an account in the Origin service, via the browser.
2. In the Origin's browser interface, Dr. Reyes navigates to the collections menu and selects her `Reyes Lab` collection.
3. Dr. Reyes creates a group `reyes_admins`, adds Christina to it, and assigns that group as the admin group of the `Reyes Lab` collection.
