...
- **Renames carry name-keyed grants with them.** Collection ACLs are keyed by *name* (group name, or the synthesised personal group `user-<username>`), not by row ID, because IdP-asserted groups from the `wlcg.groups` claim have no local row. The trade-off is that a rename could otherwise orphan every grant under the old name, where the next principal to claim that name would inherit it. So renaming a group or a user rewrites its `collection_acls` rows to the new name inside the same transaction. If the new name already appears as an ACL target (possible for a name granted to an IdP-asserted group, which has no local row), the rename is refused until those grants are revoked, rather than silently merging the two principals' grants. For the same reason, creating a local group whose name already appears as an ACL target is refused: DB-stored membership is consulted regardless of `Issuer.GroupSource`, so a local group of that name would otherwise inherit the IdP group's grants.
