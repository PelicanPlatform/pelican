...
| G4  | a collection     | any local state          | Error at the library layer: `remote object %q is a collection but recursive is not enabled`. Symmetric with the put-side directory guard (P4). Previously a non-recursive get of a collection silently wrote the origin's WebDAV listing to a local file; that behavior was easily mistaken for a successful download and is now rejected up front. |
...
| #   | Remote source is | Local destination     | Behavior                                                                                                                                                                         |
| --- | ---------------- | --------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| G5  | a collection     | an existing directory | Entries nested under `LOCAL/basename(REMOTE)/…`, matching `cp -r remote/ local/` semantics. Symmetric with P6 (recursive put of a directory into an existing remote collection). |
| G6  | a collection     | a non-existent path   | Destination path is created and entries land directly under it — matches `cp -r remote/ new_local_dir/` when `new_local_dir` doesn't yet exist.                                  |
...
## How G5/G6 decide "is the source a collection?"

Filename inference for a get into an existing local directory (G5) needs to know whether the remote source is a single object or a whole collection: an object should be dropped as `LOCAL/basename(REMOTE)`, a collection should be nested as `LOCAL/basename(REMOTE)/…`. `client.DoGet` handles that by calling `client.DoStat` up front when it detects an existing-directory destination. That call routes through the same director query as a GET, so it can be answered by any of the cache endpoints the director advertises for the namespace.

XRootD caches don't serve directory listings — a `PROPFIND` on a collection URL against a cache returns `409 Conflict`. `statHttp` recognises that specific failure (via the `errStatOnCollectionAtCache` sentinel wrapped into the error) and, if no other stat host succeeded, retries via the `X-Pelican-Collections-Url` header value (the origin's WebDAV endpoint), which does serve directory listings. Without that fallback, a second `DoGet` after a warm-up transfer against the same collection would flip from `IsCollection=true` to `false` and G5's nesting would silently regress to a flat layout mid-run.

If you touch `statHttp`, keep that fallback in place — the regression it prevents is exercised by `TestTPCSyncSkip` and the `G5`/`G6` rows in `TestObjectTransferSemantics`.
