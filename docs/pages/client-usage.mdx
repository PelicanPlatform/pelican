...
**Note:** When you set a federation discovery URL within a `pelican://` URL, within configuration files, or with the `-f` flag, do not expect the client to work if the discovery URL contains a path (e.g. `osg-htc.org/PathToMyFederation`). The client will assume that the path provided is the namespace prefix for the object which will end in failure.
...
**Namespace Prefixes:**
Each origin supports one or more *namespace prefixes*, which are analogous to the folders or directories from your computer that you use to organize files. In the example object from the OSDF mentioned earlier, the namespace prefix is `/ospool/uc-shared/public/`, and the actual object is named `/OSG-Staff/validation/test.txt`.

**Tokens and JWT:**
Some namespace prefixes are public, like `/ospool/uc-shared/public`, while others are protected (ie they require authentication). Objects in public namespaces can be downloaded by anybody, but downloading objects from protected namespaces requires you prove to the origin supporting that namespace that you are allowed to access the object. In Pelican, this is done using signed JSON Web Tokens, or *JWT*s for short. In many cases, these tokens can be generated automatically.

## The Different Pelican URL Schemes
When running client commands with a request URL, there are three different URL schemes a user can use: pelican, osdf, and stash. This page uses examples with the `pelican://` URL scheme. Below is a description of the three URL schemes and what to expect when they are used:

### The pelican:// URL Scheme:
When using the `pelican://` scheme, pelican does not know what federation to communicate with and therefore, the user must specify their federation URL either as the host-name in the URL itself or within the `-f` flag. The examples below show how you can format your request URLs in both of these ways.

### The osdf:/// URL Scheme:
When using the `osdf:///` scheme, pelican will load in the federation metadata automatically with OSDF defaults. This scheme should only be used if your federation or data can be accessed through the Open Science Data Federation. When using this scheme, you should not specify the federation URL at all, as Pelican will handle this automatically. For example:
```console
pelican object get osdf:///<federation/path/to/file> <local/path/to/file>
```
This scheme has 3 /'s after the osdf scheme when used because the user should not specify a host name in the URL, as that is typically the federation URL. However, Pelican is equipped to handle user mistakes in this area and should function properly if the user specifies only 2 /'s after osdf.

### The stash:/// URL Scheme:
This URL scheme comes from old legacy code called stashcp that Pelican was build off of and is still accepted for backwards compatibility. This URL scheme functions identically to the osdf:/// URL scheme, loading in OSDF defaults. Here is an example:
```console
pelican object get stash:///<federation/path/to/file> <local/path/to/file>
```

## Get A Public Object From Your Federation

To use the pelican client to get objects from a federation, use Pelican's `object get` sub-command
...
```console
pelican object get pelican://<federation-url>/<federation/path/to/file> <local/path/to/file>
```
Or, you can use the `-f` flag with the command, and format the request URL like so:
```console
pelican object get -f <federation-url> pelican:///</federation/path/to/file> </local/path/to/file>
```
You can try this yourself by getting the public file that was mentioned earlier from the OSDF. Using the `object get` sub command, and indicating to pelican that you're pulling from the OSDF:
...
Or:
```console
pelican object get -f https://osg-htc.org /osgconnect/public/osg/testfile.txt downloaded-testfile.txt
```

or:

```console
pelican object get pelican://osg-htc.org/osgconnect/public/osg/testfile.txt downloaded-testfile.txt
```
...
```console
pelican object get pelican://<federation-url>/<federation/path/to/file> </local/path/to/file> -t </path/to/token/file>
```
Or:
```console
pelican object get -f <federation-url> </federation/path/to/file> </local/path/to/file> -t </path/to/token/file>
```
...
## PUT an object to Your Federation
Another powerful Pelican client command is the `pelican object put` command. This command does a simple PUT request to add your object to your federation. Since the vast majority of federations will not allow public writes, we will assume for examples that we will need a token to perform these requests. Here is how you can use `pelican object put`:
```console
pelican object put <path/to/local/file> pelican://<federation-url>/<federation/path/to/destination> -t </path/to/token/file>
```
**Note:** you can also specify the federation url here with the `-f` flag, just be sure not to include it in the request URL if you decide to do so.
...
```console
pelican object copy pelican://<federation-url>/<federation/path/to/file> <local/path/to/file>
```
and to do an `object put`:
```console
pelican object copy <path/to/local/file> pelican://<federation-url>/<federation/path/to/destination> -t </path/to/token/file>
```

## Pelican Client Automatically Packing/Unpacking Tarballs
The Pelican client also has the ability to automatically compress/unpack tarballs when uploading and downloading to and from a federation. The user just needs to supply their request URL with a query. For example, to automatically unpack a tarball on downloading, one can run:
```console
pelican object copy pelican://<federation-url>/<federation/path/to/file.tar.gz>?pack=tar.gz <local/path/to/file>
```
To upload, just specify a directory and Pelican will compress it for you:
```console
pelican object copy <local/path/to/directory> pelican://<federation-url>/<federation/path/to/directory.tar.gz>?pack=tar.gz
```
Below are the following query arguments the Pelican accepts:
- `pack=auto`: For downloading, auto-detect the file format and unpack (error if it is not any detected format). For uploading, compress using `.tar.xz` by default.
- `pack=tar`: For downloading, error is thrown if specified object is not in the specified format. For uploads, create the object in the specified format.
- `pack=tar.gz`: For downloading, error is thrown if specified object is not in the specified format. For uploads, create the object in the specified format.
- `pack=zip`: For downloading, error is thrown if specified object is not in the specified format. For uploads, create the object in the specified format.
- `pack=tar.xz`: For downloading, error is thrown if specified object is not in the specified format. For uploads, create the object in the specified format.
...
- **-l or --log:** Takes a file path to specify where Pelican logs will write to instead of stdout.
