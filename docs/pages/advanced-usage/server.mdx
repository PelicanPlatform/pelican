...
The Pelican binary ships with a `pelican key create` for generating a private/public key pairs (this replaces the `pelican generate keygen` command):
...
Private key: /path/to/issuer.pem
...
To configure the Pelican server to use the newly-generated *private key*, move it into the directory specified by the `IssuerKeysDirectory` parameter:
...
1. Add the new private key file to [IssuerKeysDirectory](#multiple-keys).
2. (Optional) If you don't have the corresponding public key, [obtain it](/federating-your-data/origin#obtain-the-public-key-of-the-origin) and keep it in mind. If only see the old public key appears, wait for 60 seconds and check again.
3. Visit the Registry and download the latest registered public key set for your server under "Origins" section. Ensure the new public key is included. If it isn't, wait for 60 seconds and retry.
