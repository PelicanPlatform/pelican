...
For Pelican origin, cache, and client, they need to know where the *federation* is so that they can
attempt to join it or a access a file from it. This can be done by either manually passing configuration values (e.g. `Federation.DirectorUrl`, `Federation.RegistryUrl`, etc) or by setting the
`Federation.DiscoveryUrl` and Pelican will fetch the metadata from the endpoint automatically. `Federation.DiscoveryUrl` contains various URLs to a federation's central services including registry and director.

By default, Pelican director hosts the federation discovery endpoint. Therefore, for origins and caches in your federation, as well as Pelican clients that want to access files in your federation, their `Federation.DiscoveryUrl` value can be set to the director's public URL, i.e. `https://<your-director-hostname>:<director-web-port>` or `https://<your-director-hostname>` if the port number is simply `443`.
...
Since the director is the entrypoint of your federation, and client users uses director's public URL as the `Federation.DiscoveryUrl`, it is convenient for users to look up information related to a federation or seek help at the director's web UI. Therefore, we recommend director admin to setup contact information at the director by `Director.SupportContact` configuration parameter. If set, the information will be displayed in the director's web UI. See [Configuration](./parameters.mdx#Director-SupportContact) page for details.

```yaml filename="pelican.yaml"
Director:
  SupportContact:
    Email: <your support email>
    Url: <url to other support resources>
```

Note that either `Email` or `Url` are optional. You may leave any of them empty is not needed.
