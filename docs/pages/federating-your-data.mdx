...
* **Registry** indicates whether the origin successfully registered the namespaces prefixes it exports at the Registry
...
The **Data Exports** panel lists information about the federation prefixes that are currently being exported by the origin. It also shows the capabilities each exported namespace prefixes have.

If any of the namespace prefix registration is missing required information, there will be a warning message and a button to "Complete Registration". Note that each federation namespace prefixes is registered separately, and the Origin server itself also registers at the registry.  **It is advised that the Origin admin completes the registration as soon as they can**.
