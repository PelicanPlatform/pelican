...
Pelican has two special packaging, `OSDF` and `Server`. The `OSDF` packaging, i.e. `pelican-osdf-compat`, is for running Pelican in Open Science Data Federation (OSDF). The `Server` packaging, i.e. `pelican-server` is for running Pelican servers.

Both of the package **requires Pelican package to be installed first**. You may get the follow error message if you only install the special package:
