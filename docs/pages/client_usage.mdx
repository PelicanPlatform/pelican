...
- You are roughly familiar with Pelican client commands and terminology as demonstrated with the [Quick Start Guide](./user-quickstart-guide.mdx)
...
### The` osdf:///` URL Scheme:
When using the `osdf:///` scheme, pelican will load in the federation metadata automatically with the Open Science Data Federation (OSDF)  defaults. This scheme should only be used if your federation or data can be accessed through the OSDF. When using this scheme, you should not specify the federation URL at all, as Pelican will handle this automatically. For example:
```bash
...
```bash
pelican object get stash:///<namespace-prefix></path/to/file> <local/path/to/file>
```
...
```bash
pelican object get -f <federation-url> pelican://</namespace-prefix></path/to/file> </local/path/to/file>
```
...
If you're doing this for the very first time, Pelican will create an encrypted token wallet on your system which stores  you will be required to provide Pelican with a password for the wallet. If this isn't your first time, you will be asked to provide your already-configured password to unlock the token wallet.
...
<ExportedImage src={"/pelican/cilogon.png"} alt={"Screenshot cilogon success page"} />
...
> We are working hard to document how to generate a token from protected origins, before it’s ready. If you have any questions or need help getting a token from the origin, please email us at [help@pelicanplatform.com](mailto:help@pelicanplatform.com) and we are happy to assist you.
...
Or:
```bash
pelican object get -f <federation-url> <namespace-prefix><path/to/file> </local/path/to/file> -t </path/to/token/file>
```
...
```
> Do not share your token to others as it may contain sensitive information. Note that this token is for demonstration purposes only, and would not grant access to any files in the `/ospool/PROTECTED` namespace.
...
```bash
# Save the token to a file named “my-token”
$ echo <token> > my-token
...
Another powerful Pelican client command is the `pelican object put` command. This command does a simple PUT request to add your object to your federation. Since the vast majority of federations will not allow public writes, we will assume for the example that we will need a token to perform these requests. Here is how you can use `pelican object put`:
...
## Automatically Packing/Unpacking Tarballs
The Pelican client also has the ability to automatically compress/unpack tarballs when uploading and downloading to and from a federation. The user just needs to supply their request URL with a query. For example, to automatically unpack a tarball on downloading, run:
```bash
pelican object copy pelican://<federation-url></namespace-prefix><path/to/file/file.tar.gz>?pack=tar.gz <local/path/to/file>
```
To upload, specify a directory and Pelican will compress it for you:
```bash
pelican object copy <local/path/to/directory> pelican://<federation-url></namespace-prefix><path/to/directory.tar.gz>?pack=tar.gz
```
...
The Pelican client support a variety of command line flags that modify the client's behavior:
