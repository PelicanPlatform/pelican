...
Below are some notes on federations on top of their terminology. Here is the example URL we used in the [Getting Started: Accessing Data](./getting_started_access_data.mdx) page:
```console
...
The Pelican client has the ability to select various amount of queries specified in the remote path to an object. The queries the current Pelican client accepts are: ?pack, ?recursive, and ?directread
...
### ?recursive Query
The ?recursive query can be utilized if the desired remote object is a directory. When this query is enabled, Pelican will recursively download all files and subdirectories within the specified directory in the URL. To use this query, run:
```bash
...
> **Note about Queries:** the ?recursive and the ?directread queries do **not** require any sort of values assigned to them (i.e. pelican://some/object?recursive=true). If a value is assigned to these queries, that value will be ignored and Pelican will act as if there was no value assigned to that query (i.e. pelican://some/object?recursive=false acts the same as pelican://some/object?recursive meaning the recursive query **will** be set even if the value is set to false).
