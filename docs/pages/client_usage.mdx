...
When running client commands with a request URL, there are three different URL schemes a user can use: `pelican` and `osdf`. This page uses examples with the `pelican://` URL scheme. Below is a description of the three URL schemes and what to expect when they are used:

### The `pelican://` URL Scheme:
URLs that being with `pelican://` allow you to interact with objects from any federation simply by knowing the hostname of the federation's discovery url and the name of that object in that federation. These URLs take the form:
```bash
...
Another powerful Pelican client command is the `pelican object put` command. This command does a simple PUT request to add your object to your federation. Putting files into a federation always requires a token therefore, for the example, we will need a token to perform these requests. Here is how you can use `pelican object put`:
...
> **Note about Queries:** the ?recursive and the ?directread queries do **not** require any sort of values assigned to them (e.g. pelican://some/object?recursive=true). If a value is assigned to these queries, that value will be ignored and Pelican will act as if there was no value assigned to that query (e.g. pelican://some/object?recursive=false acts the same as pelican://some/object?recursive meaning the recursive query **will** be set even if the value is set to false).
