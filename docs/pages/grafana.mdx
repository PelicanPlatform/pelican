...
If you already had Grafana installed, skip to [Connect Grafana to Pelican](#connect-grafana-to-pelican).
...
On Linux system, you can start Grafana with systemd. For starting Grafana on other operating systems, refer to [Start Restart Grafana](https://grafana.com/docs/grafana/latest/setup-grafana/start-restart-grafana/).
...
This section describes how to connect Grafana to Pelican Prometheus instance. Grafana has built-in support for many data sources, and you will need to add a new Prometheus data source first, then configure it for Pelican. [What is Grafana data sources?](https://grafana.com/docs/grafana/latest/datasources/)
...
2. Go to the URL for Web UI and confirm it's working. The defaul URL is `https://localhost:8444`. You should be prompted to Pelican login page.

## Add a data source

1. Click **Connections** in the left-side menu. If there is no menu on the left-side, click the icon of stacked bars to toggle the menu, which is located on the top-left side of the page next to **Home**
...
Follow the instructions below to configure your Prometheus data source to work with Pelican. Each subsection is equivalent to the subsection on Grafana data source configuration page.
...
This section discribes how to configure Grafana to have proper authentication credentials to access Prometheus instance on Pelican or how to turn off Pelican authentication for Prometheus. Depending on your deployment, you may prefer one way over the other.

### Authenticated Prometheus

By default, Pelican protects the Prometheus endpoint by asking for a [JWT](https://jwt.io/introduction) to verify the user accessing the endpoint. It reads `Bearer <token>` from `Authorization` header for any HTTPS requests to `/api/v1.0/prometheus` and its subpath. You need to create a JWT using Pelican CLI and pass it to Grafana.

1. Run the following command

    * Replace `<server-web-url>` to the URL to your Pelican server Web UI. By default, it's `https://localhost:8444`.

    * Replace `--lifetime 604800` to your desired lifetime for the token. The value is in seconds. The example below generates a token with a lifetime of 7 days.
...
3. In Grafana data source configuration page, go to **Authentication** section, expand **HTTP Headers** subsection, and click **Add header**

4. Fill in `Authorization` for **Header** field and `Bearer <token>` for **Value** field. Note that there is a whitespace between `Bearer` and your token.
...
With Grafana connected to Pelican Prometheus instance, you are able to build visualization with the metrics Pelican collects. This section describes how to get started with a template dashboard configuration for a _Pelican origin server_. For a detailed guide on how to create a Grafana dashboard, refer to [Grafana documentation](https://grafana.com/docs/grafana/latest/dashboards/build-dashboards/create-dashboard/).

## Before start

1. Make sure your Pelican origin server is running

2. Download the template dashboard file from [here](../origin-dashboard-template.json)


## Modify dashboard template

There are a couple of placeholders need to be changed before you can import the dashboard template to Gafana. To do a batch replace, it is recommended that you use a text editor to find and replace placeholders to corresponding values.
...
* Replace `<YOUR-DATA-SOURCE-UID>` to the id of your Prometheus data source. You can find the ID on the data source configuration page.
    ...
    3. Batch replace `<YOUR-DATA-SOURCE-UID>` in the template dashboard file to the copied value

* Batch replace `<YOUR-ORIGIN-SERVER-WEB-URL>` to the web URL of your Pelican origin server, without the `https://` portion. Example: `localhost:8444`

* Save the changes to your template dashboard file.

## Import template file to Grafana

1. Go to home page of Grafana
...
3. Click **New** button on the top-right side. Select **Import** option in the dropdown menu.
...
<ExportedImage src={"/pelican/origin-dashboard.png"} alt={"Screenshot of Grafana page to import dashboard"} />

You may notice that some panels have `No data` warning. This should go away once you have run your server for a couple of miniutes and refresh the page. It takes about 15s for Pelican to collect the initial data points and there could be delay in visualizing the data.
