...
downloading objects happen with an HTTP "GET", uploading an object is an HTTP "PUT", discovering data uses HTTP's "HEAD" and WebDav's "PROPFIND". This architecture
means any tool that already speaks HTTP _can_ integrate with Pelican. However, Pelican's official clients -- any of our tools
...
For extra detail about Pelican's CLI, see [Getting Data With Pelican/Command Line Client](./getting-data-with-pelican/client.mdx).
...
Pelican creates a plugin that acts as the preferred transfer tool for [HTCondor](https://htcondor.org/), a tool for establishing distributed/clustered compute federations.
For more information on HTCondor's Pelican plugin, see [HTCondor's documentation](https://htcondor.readthedocs.io/en/latest/users-manual/file-transfer.html#file-transfer-using-a-url).
