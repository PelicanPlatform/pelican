...
2. In **Operating System** section, select **macOS**. In **Architectures** section, select **AMD64** or **ARM64** depending on the architecture of your machine.

3. In the list of candidates, copy the link to `pelican_Darwin_x86_64.tar.gz` if you select **AMD64**, or `pelican_Darwin_arm64.tar.gz` if you select **ARM64**.
...
5. The above command extracted the binary from the tar file. You may run the binary in the current folder, but it is recommended that you add Pelican binary to your `PATH` environment variable to allow `pelican` to be called directly from your command line.
    ...
    * Run Pelican binary from the download folder:
