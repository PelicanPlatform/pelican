...
```bash
pelican object get osdf:///<namespace-prefix></path/to/object> <local/path/to/file>
```
...
```bash
pelican object get pelican://<federation-url></namespace-prefix></path/to/object> <local/path/to/file>
```
...
pelican object get pelican://<federation-url></namespace-prefix></path/to/object> </local/path/to/file> -t </path/to/token/file>
```
...
```bash
pelican object put <path/to/local/file> pelican://<federation-url></namespace-prefix></path/to/object> -t </path/to/token/file>
```
...
```bash
pelican object copy pelican://<federation-url></namespace-prefix></path/to/object> <local/path/to/file>
```
...
```bash
pelican object get pelican://<federation-url></namespace-prefix></path/to/object/file.tar.gz>?pack=tar.gz <local/path/to/file>
```
...
```bash
pelican object get pelican://<federation-url></namespace-prefix></path/to/object>?directread <local/path/to/file>
```
...
This will only be successful when listings are enabled in the Pelican origin. To learn more about origin capabilities, go to the [Origin docs](../federating-your-data/origin.mdx).

If listings are not enabled, you will see the following error:

```
ERROR[2025-02-07T15:27:05-06:00] Failure getting pelican://osdf-director.osg-htc.org:443/nrao-ardg: failed to perform list request: Collections URL not found in director response. Are you sure there's an origin for prefix /<namespace-prefix> that supports listings?
```

## Getting Object Data From a Federation with Stat

To use the pelican client to get object data, use Pelican's `object stat` sub-command.

```bash
pelican object stat pelican://<federation-url></namespace-prefix></path/to/object>
```

This will return data in the form of:

```
Name: </namespace-path></path/to/object>
Size: <size>
ModTime: <time-last-modified>
IsCollection: false
```

This command can also be run on a collection:

```bash
pelican object stat pelican://<federation-url></namespace-prefix></path/to/collection>
```

and the data will look like:

```
Name: </namespace-path></path/to/collection>
Size: 0
ModTime: <time-last-modified>
IsCollection: true
```

## Syncing Objects with an Origin
...
To sync a local directory with a collection in the origin, use the following command:

```bash
pelican object sync pelican://<federation-url></namespace-prefix></path/to/collection> <local/directory>
```

This will put all the objects in the given source collection into the local directory, while maintaining directory structure.

### Sync Objects from a Local Directory to a Collection

To sync a collection in the origin with the files in a local directory, use the following command:

```bash
pelican object sync <local/drectory> pelican://<federation-url></namespace-prefix></path/to/collection>
```

This will upload the files from a local directory into the origin, creating namespace prefixes as necessary.

### Sync Individual Objects

The above `object sync` command can also work on the object level. To sync an object, run the following command:

```bash
pelican object sync <source> <destination>
```

Note that the destination can be either a collection or an object. If it's a collection, then the source object is placed within the collection.
