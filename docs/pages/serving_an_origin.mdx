...
If you haven't install Pelican, follow the instructions to [install pelican](/install.mdx).

For _Linux_ users, it is recommended to install Pelican using one of the package managers (RPM, APK, Deb, etc.) so that Pelican dependecies are automatically handled. You may run a [Pelican docker image](./install/docker.mdx) to serve a Pelican origin.
...
### Find the federation to join in
...
If you don't have a federation in mind, Open Science Data Federation (OSDF) is an example Pelican federation that you can join in for testing purposes. If you are interesting in serving an OSDF origin, refer to the [OSDF website](https://osg-htc.org/) for details.
...
* `<namespace_prefix>` is the namespace at which files from `<local directory>` will be made available in the federation.
...
> Also note that the one-time passcode will be refreshed every one minute. Find the latest passcode in the terminal before proceeding.
...
<ExportedImage src={"/pelican/origin-dashboard.png"} alt={"Screenshot of Pelican origin website dashboard page"} />
...
The **Federation Overview** panel lists the links to various federation services (director, registry, etc.). Note that the link to **Discovery** item is the endpoint where the metadata of a federation is located.

### For local deployment

When you hit the URL at https://localhost:8444/view/initialization/code/, You will see a warning that looks like the following (with some differences with respect to the browser):
...
2. In a **seprate terminal**, run the following command to get the data from your origin through the Pelican federation
