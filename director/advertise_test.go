...
func TestAdvertiseOSDF(t *testing.T) {
	...
	t.Run("disable-caches-from-topology", func(t *testing.T) {
		...
		require.NoError(t, param.Set(param.Topology_DisableCaches, true))
		topoServer := httptest.NewServer(http.HandlerFunc(mockTopoJSONHandler))
		defer topoServer.Close()
		require.NoError(t, param.Set(param.Federation_TopologyNamespaceUrl, topoServer.URL))
	...
	t.Run("disable-origins-from-topology", func(t *testing.T) {
		...
		require.NoError(t, param.Set(param.Topology_DisableOrigins, true))
		topoServer := httptest.NewServer(http.HandlerFunc(mockTopoJSONHandler))
		defer topoServer.Close()
		require.NoError(t, param.Set(param.Federation_TopologyNamespaceUrl, topoServer.URL))
...
func TestUpdateDowntimeFromTopology(t *testing.T) {
	...
	require.NoError(t, param.Set(param.Federation_TopologyDowntimeUrl, server.URL))
...
func TestDisableTopologyDowntime(t *testing.T) {
	...
	t.Run("disable-topology-downtime", func(t *testing.T) {
		server_utils.ResetTestState()
		serverAds.DeleteAll()
		require.NoError(t, param.Set(param.Topology_DisableDowntime, true))
		defer func() {
			server_utils.ResetTestState()
			serverAds.DeleteAll()
			filteredServersMutex.Lock()
		...
		require.NoError(t, param.Set(param.Federation_TopologyDowntimeUrl, downtimeServer.URL))
	...
	t.Run("enable-topology-downtime", func(t *testing.T) {
		server_utils.ResetTestState()
		serverAds.DeleteAll()
		require.NoError(t, param.Set(param.Topology_DisableDowntime, false))
		defer func() {
			server_utils.ResetTestState()
			serverAds.DeleteAll()
			filteredServersMutex.Lock()
