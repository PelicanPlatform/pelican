...
func registerServerAd(engineCtx context.Context, ctx *gin.Context, sType server_structs.ServerType) {
	...
	if registryPrefix == "" {
		...
		log.Warningf("Rejecting %s advertisement from %q: no registry prefix present in the ad", sType.String(), adV2.Name)
		ctx.JSON(http.StatusForbidden, server_structs.SimpleApiResp{
			Status: server_structs.RespFailed,
	...
	if !prefixTypeOK {
		log.Warningf("Rejecting %s advertisement from %q: registry prefix %q is not a %s server prefix", sType.String(), adV2.Name, registryPrefix, sType.String())
		ctx.JSON(http.StatusForbidden, server_structs.SimpleApiResp{
			Status: server_structs.RespFailed,
