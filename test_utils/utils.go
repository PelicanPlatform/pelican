...
// prepareConfigDir establishes an isolated ConfigBase for the test:
// it allocates a temp dir when ConfigBase is unset,
...
//
// It returns the resolved ConfigBase.
//
...
func prepareConfigDir(t testing.TB, o initOptions) string {
	...
	// ConfigBase (formerly ConfigDir) is the internal key for the config
	// directory; read the currently-staged value straight from viper.
	cfgDir := viper.GetString("ConfigBase")
	if cfgDir == "" {
		cfgDir = t.TempDir()
		require.NoError(t, param.ConfigBase.Set(cfgDir))
	}
	...
	// a caller that set ConfigBase might have staged a real
...
func ChownToDaemon(t testing.TB, paths ...string) {
	t.Helper()
