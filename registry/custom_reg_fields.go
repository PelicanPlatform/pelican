...
func excludePubKey(nss []server_structs.Registration) (nssNew []NamespaceWOPubkey) {
...
func InitInstConfig(ctx context.Context, egrp *errgroup.Group) error {
	...
	if param.Registry_InstitutionsUrl.GetString() != "" {
		...
		if len(institutions) > 0 {
			log.Warningf("%s and %s are both set. %s is ignored", param.Registry_Institutions.GetName(), param.Registry_InstitutionsUrl.GetName(), param.Registry_InstitutionsUrl.GetName())
