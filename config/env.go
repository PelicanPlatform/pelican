...
var (
	// Environment variable prefixes Pelican read only when invoked under an
	// OSDF- or Stash-style binary name.  Under any other name, nothing with
	// these prefixes was ever Pelican configuration.
	removedOsdfEnvPrefixes = []string{"OSDF_", "STASH_", "OSDFCP_", "STASHCP_"}

	// Individual environment variables Pelican read regardless of the binary name.
...
func warnRemovedEnv() {
	// Only an osdf* or stash*-named binary ever read the broad prefixes.
	scanPrefixes := GetPreferredPrefix() == OsdfPrefix
	...
	for _, env := range os.Environ() {
		...
		for _, prefix := range removedOsdfEnvPrefixes {
