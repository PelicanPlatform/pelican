...
func SetServerDefaults(v *viper.Viper) error {
	...
	if IsRootExecution() {
		...
		v.SetDefault(param.Server_DbLocation.GetName(), "/var/lib/pelican/server.sqlite")
		...
		v.SetDefault(param.Server_DbLocation.GetName(), filepath.Join(configDir, "server.sqlite"))
