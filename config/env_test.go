...
func TestWarnRemovedEnv(t *testing.T) {
	...
	// Under the pelican/pelican-server binary names these prefixes were never
	// read, so there is nothing to warn about; the variable may belong to
	...
	t.Run("osdf-prefixes-ignored-under-pelican-binary", func(t *testing.T) {
		...
		t.Setenv("OSDF_FEDERATION_DIRECTORURL", "https://director.example.com")
		t.Setenv("STASH_FEDERATION_DIRECTORURL", "https://director.example.com")
		t.Setenv("STASHCP_MINIMUM_DOWNLOAD_SPEED", "1024")
	...
	// The individually named variables, by contrast, were read under every
	// binary name, so they are still worth a warning.
	t.Run("individual-names-warn-under-pelican-binary", func(t *testing.T) {
		...
		require.Equal(t, 1, len(hook.Entries))
		assert.Contains(t, hook.LastEntry().Message, "OSG_DISABLE_PROXY_FALLBACK")
		assert.Contains(t, hook.LastEntry().Message, "PELICANCP_MINIMUM_DOWNLOAD_SPEED")
	})

	t.Run("removed-var-does-not-reach-viper", func(t *testing.T) {
		ResetConfig()
		hook.Reset()
		// The override is load-bearing twice over: it is what makes OSDF_
		// eligible for the warning at all, and what the old code keyed on.
		testingPreferredPrefix = OsdfPrefix
		t.Cleanup(func() { testingPreferredPrefix = "" })
		t.Setenv("OSDF_FEDERATION_DIRECTORURL", "https://director.example.com")
