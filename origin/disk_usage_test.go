...
func TestTryGetCephXattr(t *testing.T) {
	// This test will only work on systems with Ceph extended attributes
	// On most systems, this will simply verify that the function returns false

	tmpDir := t.TempDir()
	bytes, count, ok := tryGetCephXattr(tmpDir)

	// On non-Ceph systems, this should return false
	// On Ceph systems with the attributes, it should return true and valid values
	if ok {
		// If we got Ceph xattrs, verify they're reasonable
		assert.GreaterOrEqual(t, bytes, uint64(0), "Bytes should be non-negative")
		assert.GreaterOrEqual(t, count, uint64(0), "Count should be non-negative")
		t.Logf("Ceph xattrs available: %d bytes, %d files", bytes, count)
	} else {
		// This is the expected case on most test systems
		assert.Equal(t, uint64(0), bytes)
		assert.Equal(t, uint64(0), count)
		t.Log("Ceph xattrs not available (expected on non-Ceph systems)")
	}
}
