...
func TestQueueEventIDUnique(t *testing.T) {
	...
	if _, err := q.EnqueueEvent(context.Background(), event, managementTokens{}, 0); err != nil {
...
func TestQueueClaimDuePushesAttempt(t *testing.T) {
	...
	q := newPublishQueue(db)
	for i := 0; i < 3; i++ {
		_, err := q.EnqueueEvent(context.Background(), NewObjectCommitEvent("/n", "/n/x", int64(i), "", time.Now().UTC(), nil), managementTokens{}, 0)
		if err != nil {
			t.Fatalf("enqueue: %v", err)
		}
	}
...
func TestQueueScheduleRetryBumpsAttempts(t *testing.T) {
	db := newTestDB(t)
	q := newPublishQueue(db)
	row, err := q.EnqueueEvent(context.Background(), NewObjectCommitEvent("/n", "/n/x", 1, "", time.Now().UTC(), nil), managementTokens{}, 0)
	if err != nil {
		t.Fatalf("enqueue: %v", err)
	}
